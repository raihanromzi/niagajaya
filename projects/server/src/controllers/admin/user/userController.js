...
const getAllUsers = async (req, res) => {
  try {
    ...
    if (!role) {
      return res
        .status(400)
        .send(response.responseError(400, 'BAD_REQUEST', 'ADD ROLE QUERY'))
    }
    ...
    return res
      .status(500)
      .send(response.responseError(500, 'SERVER_ERROR', { message: e }))
  }
}
  ...
  try {
    ...
    if (users.length > 0) {
      return res
        .status(400)
        .send(
          response.responseError(400, 'BAD_REQUEST', 'EMAIL ALREADY REGISTERED')
        )
    }
    ...
    res
      .status(500)
      .send(response.responseError(500, 'SERVER_ERROR', 'PLEASE TRY AGAIN'))
    return
  }
}
...
const updateAdmin = async (req, res) => {
  try {
    ...
    const findAdminID = await prisma.user.findFirst({
    ...
    if (!findAdminID) {
      res
        .status(404)
        .send(response.responseError(404, 'NOT FOUND', 'ADMIN NOT FOUND'))
      return
    }
    ...
    const updatedAdmin = await prisma.user.update({
      ...
      data: {
        ...
        names: {
          update: {
            where: {
              userId_name: { userId: findAdminID.id, name },
            },
            data: { name: newName },
          },
        },
...
const deleteAdmin = async (req, res) => {
  try {
    ...
    if (deleteAdmin) {
      res
        .status(200)
        .send(response.responseSuccess(200, 'SUCCESS', 'ADMIN DELETED'))
      return
    }
  } catch (error) {
    // console.log(error);
